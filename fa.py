...
#   ※ 앞 차는 천천히 가까워지므로 한 프레임에 OBSTACLE_DELTA 이상 줄지 않음
#     정지 장애물이나 갑자기 끼어드는 물체를 구분하는 기준

# ─── 회피 동작 설정 ──────────────────────────────────────────────────────────────────────────
AVOID_STEER       = 38    # 회피 조향각 크기 (값이 클수록 급격히 회피)  권장: 20~35
AVOID_FRAMES      = 22    # 회피 조향 유지 프레임 수  (FPS ≈ 15 라면 약 1.5초)
STRAIGHT_FRAMES   = 15    # 회피 후 직진 유지 프레임 수 (장애물 완전히 지나칠 때까지)
RETURN_FRAMES     = 20    # 차선 복귀 조향 유지 프레임 수
...
def update(distance: int, modelAngle: int, autoRun: bool):
    ...
    # ------------------------------------------------------------------
    # STEP 1 : 장애물 감지 – 거리 급감 여부 체크
    #          앞 차는 점진적으로 가까워지므로 한 프레임 내 급감과 구분 가능
    # ------------------------------------------------------------------
    delta = _prevDist - distance          # 양수: 가까워짐, 음수: 멀어짐
    _prevDist = distance

    if _state not in (_ST_AVOID, _ST_STRAIGHT, _ST_RETURN):
        # distance <= 20 은 센서 노이즈(사거리 등 감지 불가 상황) → 무시
        if distance > 20 and delta >= OBSTACLE_DELTA and distance < FOLLOW_SLOW_DIST:
            # 거리가 급격히 줄고 이미 가까운 거리 → 장애물 의심 카운트 증가
            _obsCnt += 1
        else:
            # 정상 감소거나 멀어지는 경우 → 오탐 방지를 위해 카운트 서서히 감소
            _obsCnt = max(0, _obsCnt - 1)

        if _obsCnt >= OBSTACLE_CONFIRM:
            # 연속 OBSTACLE_CONFIRM 프레임 이상 감지 → 장애물 확정
            _obsCnt  = 0
            _state   = _ST_AVOID
    ...
    # ------------------------------------------------------------------
    if _state == _ST_AVOID:
        # 1단계: 장애물 옆으로 벗어나기 (회피 방향 조향)
        steerAngle = AVOID_STEER * _avoidDir
        _actLeft  -= 1
        if _actLeft <= 0:
            _state   = _ST_STRAIGHT
            _actLeft = STRAIGHT_FRAMES
            print(f'[fa] 회피 완료 → 직진 구간 시작')

    elif _state == _ST_STRAIGHT:
        # 2단계: 장애물 완전히 지나칠 때까지 직진 (모델 조향각 사용)
        steerAngle = modelAngle
        _actLeft  -= 1
        if _actLeft <= 0:
            _state   = _ST_RETURN
            _actLeft = RETURN_FRAMES
            print(f'[fa] 직진 완료 → 차선 복귀 시작')
