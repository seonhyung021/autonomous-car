...
# ─── 회피 동작 설정 ──────────────────────────────────────────────────────────────────────────
AVOID_STEER       = 28    # 회피 조향각 크기 (값이 클수록 급격히 회피)  권장: 20~35
AVOID_FRAMES      = 18    # 회피 조향 유지 프레임 수  (FPS ≈ 15 라면 약 1.2초)
...
_ST_RETURN  = 4   # 차선 복귀 중  (2단계: 레인 안으로 돌아옴)
...
def update(distance: int, modelAngle: int, autoRun: bool):
    ...
    if _state not in (_ST_AVOID, _ST_RETURN):
        if delta >= OBSTACLE_DELTA and distance < FOLLOW_SLOW_DIST:
    ...
    # ------------------------------------------------------------------
    if _state in (_ST_AVOID, _ST_RETURN):
        # 회피·복귀 중에는 속도 유지 (너무 느리면 기동 불안정)
        speedRatio = AVOID_SPEED_RATIO
        # 단, 완전 정지 거리 이하면 무조건 정지 (안전 최우선)
        if distance < FOLLOW_STOP_DIST:
            speedRatio = 0.0
            _state     = _ST_STOP
    else:
